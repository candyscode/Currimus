...
/// widget.
struct DistanceTotals: Equatable {
    var weekKm: Double
    var monthKm: Double
    var yearKm: Double
    ...
    static func current(defaults: UserDefaults = AppDefaults.shared,
        ...
        return DistanceTotals(
            ...
            monthKm: total { month.isDate($0.date, equalTo: now, toGranularity: .month) },
            yearKm: total { month.isDate($0.date, equalTo: now, toGranularity: .year) }
        )
    }
}
